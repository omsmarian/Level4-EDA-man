...
int main(int, char**)
{
	...
	string maze =
		...
		"jbbbbu+vw r      s vw+tbbbbk"      //0,0.1
		"s     +   r      s   +     r"      //0,0
		"zcccce+fg r      s fg+fcccc{"      //0,-0.1
	...
	while (!WindowShouldClose() && mqttClient.isConnected())
	{
		...
		if (gameModel.liftedRobots)
		{
			deltaTime = 0;
			gameModel.liftedRobots = 0;
		}
		...
		switch (gameModel.gameState)        //Game switch Logic 
		{
		case GameStart:
